...
//  Copyright © 2015年 Mateusz Nuckowski. All rights reserved.
//
...
@implementation JZGTableIndexView
...
- (void)setDataSource:(id<JZGTableIndexViewDataSource>)dataSource
{
    if (debug == 1) {
        NSLog(@"Running %@ '%@'",self.class, NSStringFromSelector(_cmd));
    }
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.useFont = [UIFont fontWithName:@"HelveticaNeue" size:15.0];
...
- (void)getAllItemsSize
{
    ...
    CGFloat lineHeight = self.useFont.lineHeight;
    CGFloat maxlineHeight = self.selectedItemFont.capHeight;
    CGFloat capitalLetterHeight = self.useFont.capHeight;
    CGFloat ascender = self.useFont.ascender;
    CGFloat descender = - self.useFont.descender;
    ...
    for (NSString *item in self.indexItems) {
        ...
        CGSize currentItemSize = [item sizeWithAttributes: @{NSFontAttributeName: self.useFont}];
    ...
    if (indexSize.height  > self.bounds.size.height - (self.upperMargin + self.lowerMargin) - (self.minimumGapBetweenItems * [self.indexItems count])) {
        self.useFont = [self.useFont fontWithSize:(self.useFont.pointSize - 0.1)];
//        [self getAllItemsSize];
...
- (void) initialiseAllAttributes
{
    ...
    for (NSString *item in self.indexItems) {
        ...
        if (self.itemsAligment == NSTextAlignmentCenter){
            ...
            CGSize itemSize = [item sizeWithAttributes: @{NSFontAttributeName: self.useFont}];
        ...
        if (!self.itemsAtrributes) {
            ...
            NSCache *itemAttributes = [@{@"item":item,
                                         ...
                                         @"font":(__bridge_transfer id)CTFontCreateWithName((__bridge CFStringRef)self.useFont.fontName, self.useFont.pointSize, NULL),
...
- (void) resetPosition
{
    ...
    for (NSCache *itemAttributes in self.itemsAtrributes){
        ...
        [itemAttributes setObject:(__bridge_transfer id)CTFontCreateWithName((__bridge CFStringRef)self.useFont.fontName, self.useFont.pointSize, NULL) forKey:@"font"];
    ...
    for (NSCache *itemAttributes in self.itemsAtrributes) {
        ...
        if (min && max) {
            ...
            CGFloat fontIncrease = (self.maxItemDeflection - (fabs(point.y - location.y)) *
                                    mappedAmplitude) / (self.maxItemDeflection / (self.selectedItemFont.pointSize - self.useFont.pointSize));
            
            fontIncrease = MAX(fontIncrease, 0.0);
            
            fontSize = self.useFont.pointSize + fontIncrease;
            ...
            [itemAttributes setObject:(__bridge_transfer id)CTFontCreateWithName((__bridge CFStringRef)self.useFont.fontName, fontSize, NULL) forKey:@"font"];
            ...
            if (selectedInRange || firstItemInRange || lastItemInRange) {
                ...
                if (!self.getSelectedItemsAfterPanGestureIsFinished && [self.section integerValue] != section) {
                    [self.dataSource sectionForSectionIndexTitle:self.indexItems[section] atIndex:section];
                }
...
- (void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if (self.indexItems && [self.indexItems count] > 0) {
        ...
        [self.dataSource sectionForSectionIndexTitle:self.indexItems[[self.section integerValue]] atIndex:[self.section integerValue]];
          ...
          atPoint:(CGPoint)point withAlignment:(NSTextAlignment)alignment lineBreakMode:(NSLineBreakMode)lineBreak color:(UIColor *)color
{
    ...
    CGSize newSize = [label boundingRectWithSize:size options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;
    ...
    [label drawInRect:rect withAttributes:attributes1];
