...
@interface MCTableViewCell () <UIScrollViewDelegate,  UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) MCUtilityButtonView *leftUtilityButtonsView, *rightUtilityButtonsView;
...
- (void)initializer
{
    ...
    self.cellScrollView = [[MCCellScrollView alloc] init];
    ...
    self.longPressGestureRecognizer = [[MCLongPressGestureRecognizer alloc] initWithTarget:self action:@selector(scrollViewPressed:)];
    ...
    self.leftUtilityButtonsView = [[MCUtilityButtonView alloc] initWithUtilityButtons:nil
                                                                           parentCell:self
    ...
    self.rightUtilityButtonsView = [[MCUtilityButtonView alloc] initWithUtilityButtons:nil
                                                                            parentCell:self
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if([keyPath isEqualToString:kTableViewPanState] && object == _tableViewPanGestureRecognizer)
    {
        if(_tableViewPanGestureRecognizer.state == UIGestureRecognizerStateBegan)
        {
            ...
            {
                if ([self.delegate respondsToSelector:@selector(mcipeableTableViewCellShouldHideUtilityButtonsOnSwipe:)])
                {
                    if([self.delegate mcipeableTableViewCellShouldHideUtilityButtonsOnSwipe:self])
                    {
...
{
    MCUtilityButtonTapGestureRecognizer *utilityButtonTapGestureRecognizer = (MCUtilityButtonTapGestureRecognizer *)sender;
    NSUInteger utilityButtonIndex = utilityButtonTapGestureRecognizer.buttonIndex;
    if ([self.delegate respondsToSelector:@selector(mcipeableTableViewCell:didTriggerRightUtilityButtonWithIndex:)])
    {
        [self.delegate mcipeableTableViewCell:self didTriggerRightUtilityButtonWithIndex:utilityButtonIndex];
    }
}
...
- (void)leftUtilityButtonHandler:(id)sender
{
    MCUtilityButtonTapGestureRecognizer *utilityButtonTapGestureRecognizer = (MCUtilityButtonTapGestureRecognizer *)sender;
    NSUInteger utilityButtonIndex = utilityButtonTapGestureRecognizer.buttonIndex;
    if ([self.delegate respondsToSelector:@selector(mcipeableTableViewCell:didTriggerLeftUtilityButtonWithIndex:)])
    {
        [self.delegate mcipeableTableViewCell:self didTriggerLeftUtilityButtonWithIndex:utilityButtonIndex];
    }
}
...
- (void)hideUtilityButtonsAnimated:(BOOL)animated
{
    if (_cellState != kCellStateCenter)
    {
        ...
        if ([self.delegate respondsToSelector:@selector(mcipeableTableViewCell:scrollingToState:)])
        {
            [self.delegate mcipeableTableViewCell:self scrollingToState:kCellStateCenter];
        }
    }
}
...
- (void)showLeftUtilityButtonsAnimated:(BOOL)animated {
    if (_cellState != kCellStateLeft)
    {
        ...
        if ([self.delegate respondsToSelector:@selector(mcipeableTableViewCell:scrollingToState:)])
        {
            [self.delegate mcipeableTableViewCell:self scrollingToState:kCellStateLeft];
        }
    }
}
...
- (void)showRightUtilityButtonsAnimated:(BOOL)animated {
    if (_cellState != kCellStateRight)
    {
        ...
        if ([self.delegate respondsToSelector:@selector(mcipeableTableViewCell:scrollingToState:)])
        {
            [self.delegate mcipeableTableViewCell:self scrollingToState:kCellStateRight];
        }
    }
}
...
{
#if CGFLOAT_IS_DOUBLE
    return round(CGRectGetWidth(self.rightUtilityButtonsView.frame) + self.additionalRightPadding+40.0f);
#else
    return roundf(CGRectGetWidth(self.rightUtilityButtonsView.frame) + self.additionalRightPadding+40.0f);
#endif
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    ...
    if ([self.delegate respondsToSelector:@selector(mcipeableTableViewCell:scrollingToState:)])
    {
        [self.delegate mcipeableTableViewCell:self scrollingToState:_cellState];
    }
    ...
    if (_cellState != kCellStateCenter)
    {
        if ([self.delegate respondsToSelector:@selector(mcipeableTableViewCellShouldHideUtilityButtonsOnSwipe:)])
        {
            for (MCTableViewCell *cell in [self.containingTableView visibleCells]) {
                if (cell != self && [cell isKindOfClass:[MCTableViewCell class]] && [self.delegate mcipeableTableViewCellShouldHideUtilityButtonsOnSwipe:cell]) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    if (scrollView.contentOffset.x > [self leftUtilityButtonsWidth])
    {
        ...
        {
            if (self.delegate && [self.delegate respondsToSelector:@selector(mcipeableTableViewCell:canSwipeToState:)])
            {
                BOOL shouldScroll = [self.delegate mcipeableTableViewCell:self canSwipeToState:kCellStateRight];
                if (!shouldScroll)
                {
    ...
    else
    {
        ...
        if ([self leftUtilityButtonsWidth] > 0)
        {
            if (self.delegate && [self.delegate respondsToSelector:@selector(mcipeableTableViewCell:canSwipeToState:)])
            {
                BOOL shouldScroll = [self.delegate mcipeableTableViewCell:self canSwipeToState:kCellStateLeft];
                if (!shouldScroll)
                {
                    scrollView.contentOffset = CGPointMake([self leftUtilityButtonsWidth], 0);
                }
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(mcipeableTableViewCell:didScroll:)]) {
        [self.delegate mcipeableTableViewCell:self didScroll:scrollView];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(mcipeableTableViewCellDidEndScrolling:)]) {
        [self.delegate mcipeableTableViewCellDidEndScrolling:self];
    }
}
