...
-(void)setUpChart{
	if (self.chartStyle == UUChartStyleLineDefault || self.chartStyle == UUChartStyleLinePrice || self.chartStyle == UUChartStyleLinePriceFuture) {
        if(!_lineChart){
            _lineChart = [[UULineChart alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)lineStyle:self.chartStyle];
        ...
        if ([self.dataSource respondsToSelector:@selector(chart:showHorizonLineAtIndex:)]) {
            ...
            NSInteger aa;
            if (self.chartStyle == UUChartStyleLineDefault) {
                aa = 7;
            } else if (self.chartStyle == UUChartStyleLinePrice || self.chartStyle == UUChartStyleLinePriceFuture) {
                aa = 6;
            }
            for (int i=0; i<aa; i++) {
