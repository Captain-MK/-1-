...
-(NSString *)formattedTime{
    ...
    if (!hasAMPM) { //24小时制
        ...
        if (hour >= 0 && hour <= 6) {
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:NSLocalizedString(@"yyyy-MM-dd HH:mm", @"")];
...
- (NSString *)stringWithPictureFormat {
    ...
    dispatch_once(&onceToken, ^{
        ...
        formatter.dateFormat = @"yyyy-MM-dd-HHmmssSSSS";
    });
...
+(NSString*)getCurrentTime{
    return [self convertTimeStampWithDate:[NSDate date]];
}
...
+ (NSString *)stringWithDateString:(NSString *)timeString formatterString:(NSString *)str {
    if ([timeString isEqualToString:@""]||timeString.length==0) {
        return @"";
    }
    ...
    NSString *result = [self stringWithDate:date formatterString:str];
...
+ (NSString *)getDateString:(NSString *)timeString formatter:(NSString *)formatter{
    if (!timeString || timeString.length ==0 || !formatter || formatter.length == 0) {
        return @"";
    }
    ...
    NSString *first = [timeString substringToIndex:4];
    if ([first integerValue]<2000) {
        return @"";
    }
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    NSDate *destDate= [dateFormatter dateFromString:timeString];
    return [NSDate stringWithDate:destDate formatterString:formatter];
}
...
+ (BOOL)isEarlierThanTodayBydateString:(NSString *)dateString {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [formatter setTimeZone:[NSTimeZone timeZoneWithAbbreviation:@"UTC"]];//不然会少8小时
    });
    NSString *now = [formatter stringFromDate:[NSDate date]];
    return [dateString compare:now] == NSOrderedAscending ? YES : NO;
}
