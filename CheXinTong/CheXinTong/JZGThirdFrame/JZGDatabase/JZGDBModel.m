...
#import "JZGDBModel.h"
#import "JZGDBHelper.h"

#import <objc/runtime.h>

@implementation JZGDBModel

#pragma mark - override method
+ (void)initialize
{
    if (self != [JZGDBModel self]) {
        [self createTable];
    }
}
...
{
    __block BOOL res = NO;
    JZGDBHelper *jzgDB = [JZGDBHelper shareInstance];
...
{
    //判断是否是JKBaseModel的子类
    for (JZGDBModel *model in array) {
        if (![model isKindOfClass:[JZGDBModel class]]) {
            return NO;
        }
    }
...
+ (NSArray *)findAll
{
    ...
    [jzgDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            JZGDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
